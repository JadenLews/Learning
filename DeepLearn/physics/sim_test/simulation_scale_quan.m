...
start_sim_no = 76;
nmax = 300;  % number of simulations examples you want
...
if (tt_p == 0)
    ...
    % write images as before
    imwrite(phi, phi_file);
    imwrite(P,   P_file);
    imwrite(K,   K_file);

    % continuous 0-255 versions (scaled, NOT quantized)
    phi_255 = 255 * min(max(phi, 0), 1);
    P_255   = 255 * min(max(P,   0), 1);
    K_255   = 255 * min(max(K,   0), 1);

    % quantized 0-255 versions (8-bit style, NOT compressed)
    phi_u8 = uint8(round(phi_255));
    P_u8   = uint8(round(P_255));
    K_u8   = uint8(round(K_255));

    save_file = fullfile(output_dir, strcat('State-', int2str(sim_no), '-', int2str(tt), '.mat'));
    save('-mat7-binary', save_file, ...
        'phi', 'K', 'P', 'VX', 'VY', ...
        'phi_255', 'P_255', 'K_255', ...
        'phi_u8', 'P_u8', 'K_u8');
